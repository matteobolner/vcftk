...
vcf_path="/home/pelmo/work/workspace/PRSSV_candidate_genes/data/variants/gatk/vep_annotation/VIM/snvs_biallelic.vcf"

def get_var_info_from_var(var):
    return {k:v for k,v in var.INFO}

def get_var_metadata_from_var(var):
    var_info=[var.CHROM, var.POS,var.ID,var.REF,var.ALT,var.QUAL,var.FILTER,get_var_info_from_var(var),var.FORMAT]
    return var_info

def get_var_data_from_var()
get_var_metadata_from_var(var)
...
def read_vcf_to_dataframe(vcf_path):
    ...
    allvars=[]
    for var in vcf:
        allvars.append(var_info_from_var(var))
